...
class GeometryError(errors.StructuredError, ValueError):
    """The request geometry cannot be analysed and must not be repaired silently."""

    default_code = errors.GEOMETRY_UNREADABLE
...
def load_geometry(value):
    ...
    if not isinstance(value, dict):
        raise GeometryError(
            f"unsupported geometry input of type {type(value).__name__}",
            code=errors.GEOMETRY_UNREADABLE, input_type=type(value).__name__)
    kind = value.get("type")
    if kind == "FeatureCollection":
        ...
        if len(features) != 1:
            raise GeometryError(
                ...
                code=errors.REQUEST_NOT_SINGLE_FEATURE, features=len(features))
    ...
    try:
        ...
        raise GeometryError(f"geometry is not readable as GeoJSON: {exc}",
                            code=errors.GEOMETRY_UNREADABLE,
                            geometry_type=kind) from exc
...
def validate(geometry, *, max_area_ha=MAX_AREA_HA):
    ...
    if geom.is_empty:
        raise GeometryError("geometry is empty", code=errors.GEOMETRY_EMPTY)
    if geom.geom_type not in ("Polygon", "MultiPolygon"):
        raise GeometryError(
            f"geometry must be a Polygon or MultiPolygon, got {geom.geom_type}",
            code=errors.GEOMETRY_TYPE_UNSUPPORTED, geometry_type=geom.geom_type,
    ...
    if not geom.is_valid:
        ...
        raise GeometryError(
            ...
            code=errors.GEOMETRY_SELF_INTERSECTING,
    ...
    if not (-LON_LIMIT <= lon_min and lon_max <= LON_LIMIT):
        raise GeometryError(
            ...
            code=errors.GEOMETRY_NOT_WGS84, axis="longitude",
            bounds=[lon_min, lon_max], limit=LON_LIMIT)
    if not (-LAT_LIMIT <= lat_min and lat_max <= LAT_LIMIT):
        raise GeometryError(
            f"latitude {lat_min}..{lat_max} is outside WGS84; coordinates must be "
            f"lon/lat, not lat/lon or a projected CRS",
            code=errors.GEOMETRY_NOT_WGS84, axis="latitude",
            bounds=[lat_min, lat_max], limit=LAT_LIMIT)
    area = geodesic_area_ha(geom)
    if area <= 0:
        raise GeometryError("geometry has no area", code=errors.GEOMETRY_NO_AREA)
    if area > max_area_ha:
        raise GeometryError(
            f"requested area {area:.4f} ha exceeds the {max_area_ha:.0f} ha limit; "
            f"split the contour and analyse the parts",
            code=errors.AREA_LIMIT_EXCEEDED, area_ha=round(area, 4),
...
def validate_years(year_start, year_end, *, minimum, maximum):
    ...
    if year_start < minimum or year_end > maximum:
        raise GeometryError(
            ...
            code=errors.PERIOD_OUT_OF_RANGE, year_start=year_start,
            year_end=year_end, supported_min=minimum, supported_max=maximum)
