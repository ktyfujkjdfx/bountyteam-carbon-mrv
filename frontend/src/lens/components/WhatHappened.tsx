...
/** Tab 1: what the data shows — the yearly series, the change zones and the recorded events. */
          ...
          <ul className="lens-zone-list" data-testid="lens-zone-list">
            {zones.map((zone) => (
              <li key={zone.zone_id}>
                ...
                >
                  <span>{String(zone.fact)}</span>
                  <span className="mono">{num(zone.area_ha, 1, 'га')}</span>
                </button>
              </li>
            ))}
          </ul>
              ...
              <dl className="fields">
                ...
                <div className="field">
                  ...
                  <dd>
                    ...
                    {selected.evidence_refs.length > 0 && <div className="mono small">{selected.evidence_refs.join(', ')}</div>}
                  </dd>
                </div>
              </dl>
              <p className="muted small">
                Вклад зоны показан в изменении запаса и в Eproj. Отдельное количество Q для зоны не приводится: пороги и округление
                применяются ко всему запросу и неаддитивны.
              </p>
            </article>
          )}
        </>
