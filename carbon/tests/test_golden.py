...
from carbon.tests.conftest import ALL_REQUESTS, GOLDEN_DIR, build_real_case
...
def test_the_content_hash_is_reproducible_within_a_process(request_id, areas, sample_requests):
    hashes = set()
    for _ in range(3):
        _, _, analysis, provenance = build_real_case(request_id, areas, sample_requests)
...
def test_the_content_hash_is_reproducible_in_a_fresh_interpreter():
    ...
    code = (
        "import json;"
        "from carbon.tests.conftest import build_real_case, read_csv, load_json;"
        ...
        "print(json.dumps({rid: build_passport(build_real_case(rid, areas, sr)[2], "
        "provenance=build_real_case(rid, areas, sr)[3]).content_hash "
...
def test_every_golden_result_is_built_from_verified_official_files(request_id, areas, sample_requests):
    from carbon.tests.conftest import build_real_case

    _, inputs, _, provenance = build_real_case(request_id, areas, sample_requests)
    assert provenance["all_checksums_match"] is True
    recorded = {entry["relative_path"] for entry in provenance["files"]}
    # every file RS read plus the two methodology tables every Q depends on
...
def test_the_html_report_opens_without_the_repository(areas, sample_requests, tmp_path):
    ...
    _, _, analysis, provenance = build_real_case("RU_MORDOVIA_03", areas, sample_requests)
    passport = build_passport(analysis, provenance=provenance)
    report = build_report(
...
def test_the_golden_file_declares_reviewed_rs_input():
    assert "rs.case2" in GOLDEN["note"]
    assert all(row["input_status"] == "RS_PAYLOAD" for row in GOLDEN["results"])
    assert all("PROVISIONAL_INPUT" not in row["note_codes"] for row in GOLDEN["results"])
